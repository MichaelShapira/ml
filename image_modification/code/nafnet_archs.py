# ------------------------------------------------------------------------
# Self-contained NAFNet / NAFSSR architectures for SageMaker inference.
#
# Vendored and trimmed from megvii-research/NAFNet (Apache-2.0, (c) 2022
# megvii-model) so the endpoint does not need the full BasicSR framework.
# Sources:
#   basicsr/models/archs/arch_util.py   -> LayerNorm2d, MySequential
#   basicsr/models/archs/local_arch.py  -> AvgPool2d, Local_Base (TLC)
#   basicsr/models/archs/NAFNet_arch.py -> SimpleGate, NAFBlock, NAFNet
#   basicsr/models/archs/NAFSSR_arch.py -> SCAM, NAFBlockSR, NAFNetSR
#
# Papers:
#   "Simple Baselines for Image Restoration"            (arXiv:2204.04676)
#   "NAFSSR: Stereo Image Super-Resolution Using NAFNet" (arXiv:2204.08714)
#
# The only edit vs upstream is dropping the `basicsr.utils.get_root_logger`
# import (it was used only inside commented-out DCN code) so this module is
# importable with nothing but torch + numpy.
# ------------------------------------------------------------------------
...
# ---- arch_util.py: LayerNorm2d (channel-wise LN as a 2D op) -------------
...
class MySequential(nn.Sequential):
    """nn.Sequential that forwards tuples (used by the stereo body)."""
...
# ---- local_arch.py: Test-time Local Converter (TLC) ---------------------
# Replaces global AdaptiveAvgPool2d(1) with a sliding local average so a model
# trained on small patches generalizes to full-resolution inputs at test time.
# (ref. Chu et al., "Revisiting Global Statistics Aggregation", arXiv:2112.04491)
...
# ---- NAFNet_arch.py: denoise / deblur backbone --------------------------
...
# ---- NAFSSR_arch.py: stereo super-resolution ----------------------------
...
class SCAM(nn.Module):
    ...
    def __init__(self, c):
        ...
        self.l_proj1 = nn.Conv2d(c, c, 1, stride=1, padding=0)
        self.r_proj1 = nn.Conv2d(c, c, 1, stride=1, padding=0)
        self.beta = nn.Parameter(torch.zeros((1, c, 1, 1)), requires_grad=True)
        self.gamma = nn.Parameter(torch.zeros((1, c, 1, 1)), requires_grad=True)
        self.l_proj2 = nn.Conv2d(c, c, 1, stride=1, padding=0)
        self.r_proj2 = nn.Conv2d(c, c, 1, stride=1, padding=0)
...
class NAFNetSR(nn.Module):
    def __init__(self, up_scale=4, width=48, num_blks=16, img_channel=3, drop_path_rate=0.,
                 drop_out_rate=0., fusion_from=-1, fusion_to=-1, dual=False):
        super().__init__()
        self.dual = dual
        self.intro = nn.Conv2d(img_channel, width, 3, padding=1, stride=1, groups=1, bias=True)
        ...
        self.up = nn.Sequential(
            nn.Conv2d(width, img_channel * up_scale ** 2, 3, padding=1, stride=1, groups=1, bias=True),
...
# ---- registry: task -> (builder, default weight filename) ---------------
# Widths/blocks match the official width64 (NAFNet) and NAFSSR-L configs.

def build_denoise():
    # NAFNet-SIDD-width64
    return NAFNet(img_channel=3, width=64, middle_blk_num=12,
                  enc_blk_nums=[2, 2, 4, 8], dec_blk_nums=[2, 2, 2, 2])


def build_deblur():
    # NAFNet-GoPro-width64 / NAFNet-REDS-width64 share this config
    return NAFNet(img_channel=3, width=64, middle_blk_num=1,
                  enc_blk_nums=[1, 1, 1, 28], dec_blk_nums=[1, 1, 1, 1])


def build_stereo_sr(up_scale=4):
    # NAFSSR-L (23.8M). Plain NAFNetSR (no TLC) — checkpoints load identically
    # since the TLC AvgPool2d carries no learnable parameters.
    return NAFNetSR(up_scale=up_scale, width=128, num_blks=128, dual=True,
                    fusion_from=0, fusion_to=1000)
